...
class SixPieceBurrPuzzle:
    ...
    def validate_dimensions(self):
        ...
        diff = self.max - self.mid
        if diff < 2:
            raise ValueError(f"零件尺寸最大的(当前为{self.max})与次大的(当前为{self.mid})之间相差不足 2 个单位")
        elif diff > 2:
            raise ValueError(f"零件尺寸最大的(当前为{self.max})与次大的(当前为{self.mid})之间相差超过 2 个单位")

    # 初始化三维数组
    def init_space(self):
        return [[[[] for _ in range(self.max)] for _ in range(self.max)] for _ in range(self.max)]
    ...
    def build_parts(self):
        for part_name, part_info in self.parts.items():
            ...
            for l in range(self.max // 2 - self.L // 2, self.max // 2 + self.L // 2):
                for w in range(self.max // 2 + min(0, param[3] * self.W), self.max // 2 + max(0, param[3] * self.W)):
                    for h in range(self.max // 2 - self.H // 2, self.max // 2 + self.H // 2):
                        idx = [0, 0, 0]
                        idx[param[0]] = l # H方向
                        idx[param[1]] = w # W方向
                        idx[param[2]] = h  # L方向
    ...
    def create_scad_file(self, filename):
        with open(filename, 'w') as f:
            ...
            offset_dist = 1.5 * self.max * self.unit_cube
            offset_map = {}
            for part_name, part_info in self.parts.items():
...
if __name__ == "__main__":
    ...
    W = 4
    H = 8
    puzzle_show = SixPieceBurrPuzzle(L, W, H)
